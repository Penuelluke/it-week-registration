...
const checkRegistrationDeadline = () => {
  const deadline = new Date(2025, 3, 18, 23, 59, 0); // April is month 3 (0-indexed)
  const now = new Date();
  return now > deadline;
};
...
export function RegistrationForm() {
  ...
  const [isDeadlinePassed, setIsDeadlinePassed] = useState(checkRegistrationDeadline());

  // Update deadline check periodically (every minute)
  useEffect(() => {
    const interval = setInterval(() => {
      setIsDeadlinePassed(checkRegistrationDeadline());
    }, 60000);
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      ctx.fillText(`SYSTEM STATUS: ${isDeadlinePassed ? 'OFFLINE' : 'ONLINE'}`, 20, 30);
  ...
  }, [scrollProgress, isDeadlinePassed]);
    ...
    if (isDeadlinePassed) {
      return (
        <div className="text-center py-12 relative z-10">
          <div className="inline-flex items-center justify-center p-4 rounded-full bg-red-900/50 border border-red-500/30 mb-4">
            <Clock size={24} className="text-red-400" />
          </div>
          <h3 className="text-xl font-medium text-red-300">
            Registration Closed
          </h3>
          <p className="mt-2 text-gray-400 max-w-md mx-auto">
            The registration deadline was April 18, 2025 at 11:59 PM.
          </p>
    ...
    <div className="min-h-screen bg-gray-950 text-white relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 py-6">
        <div className="text-center mb-10 sm:mb-14 relative">
          ...
          <div className={`inline-flex flex-wrap items-center justify-center gap-2 px-4 py-2 mb-4 ${
            isDeadlinePassed ? 'bg-red-900/50 border-red-500/30 text-red-300' : 'bg-gray-900/50 border-purple-500/30 text-purple-300'
          } border rounded-full text-xs sm:text-sm backdrop-blur-sm w-full sm:w-auto text-center`}>
            <Clock size={16} className="shrink-0" />
            <span className="whitespace-normal">
              {isDeadlinePassed ? 'Registration Closed' : 'Registration Deadline: April 18, 2025 @ 11:59 PM'}
            </span>
          </div>
        ...
        <div className="grid gap-8 lg:grid-cols-2 relative">
          ...
          <div className="space-y-6">
            <h2 className="text-xl sm:text-2xl font-semibold text-transparent bg-clip-text bg-gradient-to-r from-purple-300 to-cyan-300 px-2">
              {isDeadlinePassed ? 'Past Events' : 'Available Events'}
            </h2>
            <div className="grid sm:grid-cols-2 gap-4">
              {EVENTS.map(event => (
                <div key={event.id} className={isDeadlinePassed ? 'opacity-60 cursor-not-allowed' : ''}>
                  <EventCard
                    event={event}
          ...
          >
            ...
            <div className="flex items-center gap-3 mb-6">
              <div className={`p-2 rounded-lg ${
                selectedEventData && !isDeadlinePassed
              ...
              <h2 className="text-xl sm:text-2xl font-semibold text-white">
                {selectedEventData ? (
                  <>
                    {isDeadlinePassed ? 'Event Closed: ' : 'Register for '}
